...
export interface RemoveBgOptions { apiKey?: string; }
export interface RemoveBgClientResult { imageUrls: string[]; }

export async function removeBackground(imageUrl: string, options?: RemoveBgOptions): Promise<RemoveBgClientResult> {
  ...
  const createRes = await fetch(`${KIE_API_BASE}/createTask`, {
    method: "POST", headers,
    body: JSON.stringify({ model: MODEL, input: { image: imageUrl } }),
  });
  ...
  for (let i = 0; i < MAX_POLLS; i++) {
    ...
    if (state === "fail") throw new Error(`Kie.ai remove-bg failed: ${pollData.data?.failMsg ?? "unknown"}`);
  }
