...
| `daily-auto-publish.md` | approved 브리프 quality check + scheduled/published 전환 (`publish:auto-dry` -> `publish:auto`) | 매일 1회, drift guard 이후 |
...
- `daily-auto-publish`는 approved 브리프를 quality gate 뒤에만 예약/발행한다.
