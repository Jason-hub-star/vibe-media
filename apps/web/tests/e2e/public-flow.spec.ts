...
test("public flow moves from home to radar, brief, sources, and newsletter", async ({ page }) => {
  await page.goto("/");
  await expect(page.getByRole("heading", { name: /ai 뉴스를 빠르게, 한국어 브리프로\./i })).toBeVisible();
  ...
  await expect(page.getByRole("heading", { name: /ai 뉴스 한국어 브리프 아카이브/i })).toBeVisible();

  await navigation.getByRole("link", { name: "Sources" }).click();
  await expect(page.getByRole("heading", { name: /브리프 작성에 활용하는 공식 소스 목록/i })).toBeVisible();
