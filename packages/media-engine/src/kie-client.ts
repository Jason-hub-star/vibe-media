...
export async function generateImageWithKie(prompt: string, options?: KieGenerateOptions): Promise<KieGeneratedResult> {
  ...
  const createRes = await fetch(`${KIE_API_BASE}/createTask`, {
    method: "POST", headers,
    body: JSON.stringify({ model: MODEL, input: { prompt, aspect_ratio: options?.aspectRatio ?? "1:1", resolution: options?.resolution ?? "1K", output_format: "jpg", google_search: false, image_input: options?.imageInput ?? [] } }),
  });
  ...
  if (createData.code !== 200 || !createData.data?.taskId) throw new Error(`Kie.ai createTask error: ${createData.msg}`);
  const { taskId } = createData.data;

  for (let i = 0; i < MAX_POLLS; i++) {
    await new Promise((r) => setTimeout(r, POLL_INTERVAL));
    const pollRes = await fetch(`${KIE_API_BASE}/recordInfo?taskId=${taskId}`, { headers });
    if (!pollRes.ok) continue;
    const pollData = (await pollRes.json()) as { code: number; data?: { state: string; resultJson?: string; failMsg?: string | null } };
    ...
    if (state === "fail") throw new Error(`Kie.ai generation failed: ${pollData.data?.failMsg ?? "unknown"}`);
  }
